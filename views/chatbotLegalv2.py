...
try:
    ...
    print("Make sure Docker is running and Redis container is started with: docker-compose up -d")
...
def create_new_chat() -> str:
    existing_chats = get_chat_list()
    new_chat_name = f"Chat {len(existing_chats) + 1}"
...
def get_chat_list() -> list:
    """Get all chat names from Redis using scan_iter for reliability."""
    try:
        # Use scan_iter for better performance and reliability
        keys = [key for key in redis_client.scan_iter(match="Chat*")]
        return keys
    except Exception as e:
